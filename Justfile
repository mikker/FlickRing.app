...
build:
	xcodebuild -scheme FlickRing -configuration Debug build

run:
	#!/usr/bin/env bash
	set -euo pipefail
	settings=$(xcodebuild -scheme FlickRing -configuration Debug -showBuildSettings)
	...
	xcodebuild -scheme FlickRing -configuration Debug build
	pkill -x "FlickRing β" 2>/dev/null || true
...
archive:
	bash -lc 'set -euo pipefail; rm -rf build/FlickRing.xcarchive; xcodebuild -scheme FlickRing -configuration Release archive -destination "generic/platform=macOS" -archivePath build/FlickRing.xcarchive ARCHS="arm64 x86_64" ONLY_ACTIVE_ARCH=NO'
